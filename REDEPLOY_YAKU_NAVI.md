# 🚀 yaku-navi.com 完全再デプロイ手順（PM2強化版）

**対象サーバー**: yaku-navi.com  
**作成日**: 2026年1月25日  
**所要時間**: 約30-40分

---

## 📋 概要

### 現在の状況
- ❌ **yaku-navi.com**: 502エラー（Bad Gateway）
- ❌ PM2プロセスが停止または異常終了している可能性

### 実施内容
1. VPSサーバーに再アップロード
2. PM2自動再起動設定の強化
3. サーバー再起動時の自動起動設定
4. 動作確認

### 期待される効果
- ✅ 502エラーの解消
- ✅ PM2の自動再起動が強化される
- ✅ サーバー再起動後も自動で起動
- ✅ メモリ制限超過時の自動再起動
- ✅ 毎日定期的に自動再起動（午前4時）

---

## 🔧 Step 1: ローカル環境の準備（5分）

### 1-1. 最新のコードを確認

```bash
cd /Users/soedakei/pharmacy-platform

# 現在のブランチ確認
git branch

# 最新の変更をコミット
git add .
git commit -m "PM2自動再起動設定強化 & 502エラー対応"
```

### 1-2. 必要なファイルの確認

以下のファイルが存在することを確認：

```bash
ls -la ecosystem.config.js
ls -la backend/package.json
ls -la package.json
ls -la next.config.ts
```

---

## 🌐 Step 2: VPSサーバーへのアップロード（10分）

### ⚙️ Step 2-0: SSH設定ファイルの設定（初回のみ・パスワード入力回避）

SSH鍵認証でパスワード入力を回避するため、SSH設定ファイルを設定します：

```bash
# SSH設定ファイルを作成（既に作成済みの場合はスキップ）
cat >> ~/.ssh/config << 'EOF'

# yaku-navi.com 設定
Host yaku-navi
    HostName 162.43.8.168
    User root
    IdentityFile ~/.ssh/id_ed25519
    IdentitiesOnly yes
    StrictHostKeyChecking no
    UserKnownHostsFile ~/.ssh/known_hosts

Host yaku-navi.com
    HostName 162.43.8.168
    User root
    IdentityFile ~/.ssh/id_ed25519
    IdentitiesOnly yes
    StrictHostKeyChecking no
    UserKnownHostsFile ~/.ssh/known_hosts
EOF

# パーミッションを設定
chmod 600 ~/.ssh/config
```

**設定後は、`yaku-navi` または `yaku-navi.com` という短い名前で接続できます（パスワード不要）**

### 方法A: rsyncでアップロード（推奨・パスワード不要）

```bash
# プロジェクトルートから実行
cd /Users/soedakei/pharmacy-platform

# バックエンドをアップロード（パスワード不要）
rsync -avz --exclude 'node_modules' --exclude '.git' \
  backend/ yaku-navi:/var/www/pharmacy-platform/backend/

# フロントエンドのファイルをアップロード（パスワード不要）
rsync -avz --exclude 'node_modules' --exclude '.git' --exclude '.next' \
  app/ yaku-navi:/var/www/pharmacy-platform/app/
rsync -avz --exclude 'node_modules' --exclude '.git' \
  components/ yaku-navi:/var/www/pharmacy-platform/components/
rsync -avz --exclude 'node_modules' --exclude '.git' \
  lib/ yaku-navi:/var/www/pharmacy-platform/lib/
rsync -avz --exclude 'node_modules' --exclude '.git' \
  public/ yaku-navi:/var/www/pharmacy-platform/public/

# ecosystem.config.jsをアップロード（重要・パスワード不要）
scp ecosystem.config.js yaku-navi:/var/www/pharmacy-platform/

# package.jsonをアップロード（パスワード不要）
scp package.json yaku-navi:/var/www/pharmacy-platform/
scp package-lock.json yaku-navi:/var/www/pharmacy-platform/
scp next.config.ts yaku-navi:/var/www/pharmacy-platform/
scp tsconfig.json yaku-navi:/var/www/pharmacy-platform/
scp tailwind.config.ts yaku-navi:/var/www/pharmacy-platform/
scp postcss.config.mjs yaku-navi:/var/www/pharmacy-platform/
scp backend/package.json yaku-navi:/var/www/pharmacy-platform/backend/
scp backend/package-lock.json yaku-navi:/var/www/pharmacy-platform/backend/
```

### 方法B: FTPでアップロード

1. FileZillaなどのFTPクライアントを起動
2. yaku-navi.comに接続
3. 以下のファイル/フォルダをアップロード：
   - `ecosystem.config.js` ✅ **重要**
   - `backend/src/`
   - `backend/package.json`
   - `package.json`
   - `.next/`（ビルド済みの場合）

---

## 🔐 Step 3: VPSサーバーにSSH接続（1分）

SSH設定ファイルを設定済みの場合、以下の短いコマンドで接続できます（パスワード不要）：

```bash
ssh yaku-navi
```

または

```bash
ssh yaku-navi.com
```

**注意**: SSH設定ファイルを設定していない場合は、以下のコマンドを使用：
```bash
ssh root@162.43.8.168
```

---

## 🛠️ Step 4: プロジェクトディレクトリに移動（1分）

```bash
cd /var/www/pharmacy-platform

# ディレクトリ構造を確認
ls -la
```

**確認項目:**
- ✅ `ecosystem.config.js` が存在する
- ✅ `backend/` ディレクトリが存在する
- ✅ `package.json` が存在する

---

## 📦 Step 5: 依存関係のインストール（5分）

### 5-1. バックエンドの依存関係

```bash
cd /var/www/pharmacy-platform/backend
npm install --production
```

### 5-2. フロントエンドの依存関係

```bash
cd /var/www/pharmacy-platform
npm install --production
```

### 5-3. フロントエンドのビルド（ローカルでビルドしていない場合）

```bash
npm run build
```

**注意**: ビルドには5-10分かかる場合があります。

---

## 🔄 Step 6: PM2プロセスの完全リセット（3分）

### 6-1. 現在のPM2プロセスを全て停止・削除

```bash
cd /var/www/pharmacy-platform

# 現在のプロセス確認
pm2 status

# 全プロセスを停止
pm2 stop all

# 全プロセスを削除
pm2 delete all

# PM2プロセスリストをクリア
pm2 flush
```

### 6-2. PM2リストの確認

```bash
pm2 list
```

**期待される出力:**
```
┌─────┬──────┬─────────┬─────────┬──────┬──────┐
│ id  │ name │ version │ mode    │ pid  │ status│
├─────┼──────┼─────────┼─────────┼──────┼──────┤
│     │      │         │         │      │      │
└─────┴──────┴─────────┴─────────┴──────┴──────┘
```

（空のリストが表示されればOK）

---

## 🚀 Step 7: PM2で新しい設定で起動（3分）

### 7-1. ecosystem.config.jsで起動

```bash
cd /var/www/pharmacy-platform

# 新しい設定で起動
pm2 start ecosystem.config.js

# ステータス確認
pm2 status
```

**期待される出力:**
```
┌─────┬───────────────────────┬─────────┬─────────┬──────┬────────┐
│ id  │ name                  │ version │ mode    │ pid  │ status │
├─────┼───────────────────────┼─────────┼─────────┼──────┼────────┤
│ 0   │ pharmacy-backend      │ N/A     │ fork    │ XXXX │ online │
│ 1   │ pharmacy-frontend     │ N/A     │ fork    │ XXXX │ online │
└─────┴───────────────────────┴─────────┴─────────┴──────┴────────┘
```

両方とも **`online`** になっていることを確認！

### 7-2. ログを確認

```bash
# バックエンドログ
pm2 logs pharmacy-backend --lines 20

# フロントエンドログ
pm2 logs pharmacy-frontend --lines 20
```

**エラーがないことを確認**

---

## 💾 Step 8: PM2設定を保存（2分）

### 8-1. 現在のプロセスリストを保存

```bash
pm2 save
```

**出力:**
```
[PM2] Saving current process list...
[PM2] Successfully saved in /root/.pm2/dump.pm2
```

### 8-2. サーバー起動時の自動起動を設定

```bash
# systemdの起動スクリプトを生成
pm2 startup systemd

# 表示されたコマンドを実行（例）
sudo env PATH=$PATH:/usr/bin /usr/lib/node_modules/pm2/bin/pm2 startup systemd -u ユーザー名 --hp /home/ユーザー名
```

**注意**: 表示されたコマンドをそのまま実行してください。

### 8-3. 設定を再保存

```bash
pm2 save
```

---

## ✅ Step 9: 動作確認（5分）

### 9-1. プロセス状態の確認

```bash
pm2 status
```

**確認項目:**
- ✅ 両方とも `online`
- ✅ `restart` が 0 または少ない数値
- ✅ `uptime` が増えている

### 9-2. ポート確認

```bash
# バックエンドポート
netstat -tuln | grep 3001

# フロントエンドポート
netstat -tuln | grep 3000
```

**期待される出力:**
```
tcp6       0      0 :::3001                 :::*                    LISTEN
tcp6       0      0 :::3000                 :::*                    LISTEN
```

### 9-3. curlでAPI確認

```bash
# バックエンドAPI
curl http://localhost:3001/

# フロントエンド
curl http://localhost:3000/
```

### 9-4. ブラウザで確認

1. **https://yaku-navi.com** にアクセス
2. トップページが表示されることを確認
3. **https://yaku-navi.com/invoice-issued** にアクセス
4. 請求書発行ページが表示されることを確認

---

## 🔍 Step 10: PM2詳細設定の確認（3分）

### 10-1. PM2設定の確認

```bash
pm2 show pharmacy-backend
pm2 show pharmacy-frontend
```

**確認項目:**
- ✅ `autorestart`: true
- ✅ `max_memory_restart`: 設定されている
- ✅ `max_restarts`: 15
- ✅ `cron_restart`: 設定されている（毎日午前4時）

### 10-2. PM2ログローテーション設定（オプション）

```bash
# PM2ログローテーションモジュールをインストール
pm2 install pm2-logrotate

# 設定
pm2 set pm2-logrotate:max_size 10M
pm2 set pm2-logrotate:retain 7
pm2 set pm2-logrotate:compress true
```

---

## 🎯 Step 11: 最終確認とモニタリング（5分）

### 11-1. PM2モニタリング

```bash
# リアルタイムモニタリング
pm2 monit
```

**確認項目:**
- CPU使用率
- メモリ使用量
- リアルタイムログ

### 11-2. 5分間の安定性確認

```bash
# 5分待つ
sleep 300

# プロセス状態を確認
pm2 status
```

**確認項目:**
- ✅ 両方とも `online` のまま
- ✅ `restart` が増えていない

### 11-3. ログに異常がないか確認

```bash
pm2 logs --lines 50
```

---

## 📊 新しいPM2設定の詳細

### 強化された自動再起動設定

| 設定項目 | 値 | 説明 |
|---------|-----|------|
| `autorestart` | true | プロセス停止時に自動再起動 |
| `max_restarts` | 15 | 最大再起動回数（10→15に増加） |
| `restart_delay` | 5000ms | 再起動までの待機時間 |
| `exp_backoff_restart_delay` | 100ms | 指数バックオフ再起動 |
| `max_memory_restart` | 500M/1G | メモリ制限超過時に再起動 |
| `min_uptime` | 10s | 最小稼働時間 |
| `kill_timeout` | 5000ms | プロセス終了タイムアウト |
| `cron_restart` | 0 4 * * * | 毎日午前4時に定期再起動 |

### 定期的な自動再起動

- **バックエンド**: 毎日午前4:00に自動再起動
- **フロントエンド**: 毎日午前4:05に自動再起動

メモリリークやリソース蓄積を防ぐための予防措置。

---

## 🚨 トラブルシューティング

### エラー1: `pm2 command not found`

**原因**: PM2がインストールされていない

**解決策**:
```bash
sudo npm install -g pm2
```

### エラー2: プロセスがすぐに停止する

**原因**: 依存関係のエラー、ポート競合

**解決策**:
```bash
# ログを確認
pm2 logs pharmacy-backend --err --lines 50

# ポート確認
lsof -i :3001
lsof -i :3000

# 依存関係を再インストール
cd /var/www/pharmacy-platform/backend
rm -rf node_modules
npm install --production
```

### エラー3: `502 Bad Gateway` が継続

**原因**: Nginxの設定、バックエンドポートの不一致

**解決策**:
```bash
# Nginx設定を確認
sudo nginx -t

# Nginx再起動
sudo systemctl restart nginx

# バックエンドポート確認
netstat -tuln | grep 3001
```

### エラー4: メモリ不足

**原因**: VPSのメモリが不足

**解決策**:
```bash
# メモリ使用状況確認
free -h

# スワップ領域の作成（必要な場合）
sudo fallocate -l 2G /swapfile
sudo chmod 600 /swapfile
sudo mkswap /swapfile
sudo swapon /swapfile
```

---

## 📝 デプロイ後のチェックリスト

### 即座に確認

- [ ] PM2プロセスが `online` になっている
- [ ] ポート3001と3000が LISTEN 状態
- [ ] https://yaku-navi.com にアクセスできる
- [ ] https://yaku-navi.com/invoice-issued にアクセスできる

### 5分後に確認

- [ ] PM2プロセスが `online` のまま
- [ ] `restart` カウントが増えていない
- [ ] エラーログが出ていない

### 30分後に確認

- [ ] PM2プロセスが安定している
- [ ] メモリ使用量が正常範囲内
- [ ] ログインなど各機能が正常動作

### 翌日確認

- [ ] 午前4時の定期再起動が実行された
- [ ] 再起動後も正常動作している

---

## 🔄 今後の更新方法

### コード更新時

```bash
# 1. ローカルでコミット
git add .
git commit -m "更新内容"

# 2. サーバーにアップロード
rsync -avz --exclude 'node_modules' \
  backend/ ユーザー名@yaku-navi.com:/var/www/pharmacy-platform/backend/

# 3. SSH接続
ssh ユーザー名@yaku-navi.com

# 4. PM2で再起動
cd /var/www/pharmacy-platform
pm2 restart all

# 5. 確認
pm2 status
```

---

## 📊 PM2コマンド早見表

```bash
# 状態確認
pm2 status

# ログ確認
pm2 logs
pm2 logs pharmacy-backend --lines 100

# 再起動
pm2 restart all
pm2 restart pharmacy-backend

# 停止
pm2 stop all

# モニタリング
pm2 monit

# 詳細情報
pm2 show pharmacy-backend

# 設定保存
pm2 save

# プロセス削除
pm2 delete all
```

---

## 🎊 完了！

この手順を実行すると、以下が実現されます：

1. ✅ **502エラーの解消**
2. ✅ **PM2自動再起動の強化**
3. ✅ **サーバー再起動時の自動起動**
4. ✅ **メモリ制限超過時の自動再起動**
5. ✅ **毎日定期的な自動再起動**
6. ✅ **より安定したサービス運用**

---

**作成者**: AI Assistant  
**作成日**: 2026年1月25日  
**最終更新**: 2026年1月25日

